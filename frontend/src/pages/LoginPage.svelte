...
<script>
    import { KeyRound, TerminalIcon, User } from "@lucide/svelte";
...
<div class="flex h-screen overflow-hidden">
    ...
    >
        ...
        <div class="flex items-center gap-3 relative z-10">
            ...
            <div>
                <span class="text-white font-bold text-lg tracking-wide block"
                    >{AppName}</span
                >
                <span class="text-xs text-zinc-500 font-mono">{Version}</span>
            </div>
        </div>
        ...
        >
            <div>
                <span class="text-[10px] uppercase text-zinc-500 tracking-wider"
                    >Global Ping</span
                >
                <p class="text-sm font-bold text-emerald-400 mt-0.5">14ms</p>
            </div>
            <div>
                <span class="text-[10px] uppercase text-zinc-500 tracking-wider"
                    >Uptime</span
                >
                <p class="text-sm font-bold text-zinc-200 mt-0.5">99.998%</p>
            </div>
            <div>
                <span class="text-[10px] uppercase text-zinc-500 tracking-wider"
                    >Requests / sec</span
                >
                <p class="text-sm font-bold text-indigo-400 mt-0.5">142.5k</p>
            </div>
        </div>
    </div>
        ...
        <div class="w-full max-w-sm space-y-6">
            ...
            <form
                ...
                onsubmit={async (event) => {
                    ...
                    const { data, success, errors } = await ResourceApis.create(
                        Resources.LOGIN,
                        form,
                    );
                    formErrors = errors
            ...
            >
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            class="{formErrors?.email ? 'border-red': ''} w-full pl-10 pr-4 py-2 text-sm bg-zinc-900 border border-zinc-800 rounded-lg focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-zinc-100 placeholder-zinc-700 transition-colors"
                        />
                        <p class="text-red-500 py-1 first-letter:uppercase">{formErrors?.email}</p>
                    </div>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        />
                        <p class="text-red-500 py-1 first-letter:uppercase">{formErrors?.password}</p>
                        <button
                            type="button"
                ...
                <button
                    type="submit"
                    class="w-full py-2 bg-indigo-600 text-white rounded-lg text-sm font-semibold hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-zinc-950 shadow-lg shadow-indigo-600/20 transition-all flex items-center justify-center gap-2 group"
                >
                    <span>Proceed</span>
                    <i
